...
# -------------------------------연 습 문 제------------------------------------
# 연습문제 9.
# 여아신생아.txt 파일을 읽고 신생아 몸무게가 2800보다 클 것이다라는 가설에
# 대한 가설 검정 수행
# 1) data loading
...
# 3) T분포에서의 기각역과 채택역 (유의수준 5%)
qt(p = 1 - 0.05, df = v_n - 1)
# [ ..., 1.739607  ]
  
# 4) 검정통계량
T = (xbar - mu) / (s/sqrt(n))
T* = (v_xbar - mu) / (v_s/sqrt(v_n))    # 2.233188

# 채택역인 [ INF, 1.739607  ]에 검정통계량 값이 포함되지 않으므로 H0 기각!

# 5) 유의 확률 계산
P(Z>Z*)      # Z분포에서의 왼쪽검정의 유의 확률
P(T>T*)      # T분포에서의 왼쪽검정의 유의 확률

P(T>2.233188)
1 - pt(2.233188,       # 누적확률을 계산할 임계값
       df = v_n-1)     # 0.01963421 <<< 0.05
# H0 기각!
...
# 모평균의 95% 신뢰구간이 [ 2873.477, INF ]에 H0의 mu인 2800이 포함되어 있지 않으므로
# H0 기각!
...
data:  v1
t = 2.2332, df = 17, p-value = 0.01963    # <<< 0.05
