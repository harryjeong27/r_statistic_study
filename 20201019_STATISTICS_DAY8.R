...
-1.96 * sigma/sart(n) <= xbar - mu <= 1.96 * sigma/sqrt(n)
...
# [ 12.51035, ... ], mu = 12.5가 신뢰구간에 포함되어 있지 않으므로
# H0 기각!!!
...
xbar ~ N(mu, (sigma/sqrt(n))^2)
Z ~ N(0, 1)
(xbar - mu) / (sigma/sqrt(n)) ~ N(0, 1)    # Z검정 => 표준정규분포를 따르다
(xbar - mu) / (s/sqrt(n)) ~ T(n-1)         # T검정 => T분포를 따른다
# s = 표본의 표준편차

# (xbar - mu) / (s/sqrt(n)) ~ T(n-1)
# 평균이 10, 분산이 4(표준편차가 2)인 정규분포를 따르는 모집단으로부터 1000개의
# 표본을 추출, 표본평균이 갖는 분포를 시각화
...
# 2-2) sigma를 알고 있을 경우 이론적 분포 (정규분포에 근사)
xbar ~ N(10, 2/sqrt(100))
(xbar - mu) / (sigma / sqrt(n)) ~ N(0, 1)

# 2-3) xbar ~ N(10, 2/sqrt(100))
vx1 <- seq(min(vmean), max(vmean), 0.01)
vy1 <- dnorm(vx1, mean = 10, sd = 2/sqrt(100))

# 3) sigma를 모를 경우 이론적 분포 (T분포에 근사)
(xbar - mu) / (s/sqrt(n)) ~ T(0, 1)

(xbar - mu) / (s/sqrt(n)) ~ T(n-1)

vt <- (vmean - 10) / (vsd / sqrt(100))
dev.new()
hist(vt, probability = T, ylim = c(0, 0.4))    # 표준화된 확률변수 T의 실제 분포
...
# => 자유도가 작을수록 꼬리가 두껍고, 커질수록 꼬리가 얇으면서 중앙의 
#    밀집도가 높아진다
# ------------------------------------------------------------------------------